...
class Tateti:
    def __init__(self):
        ...
        root = Tk()
        root.title("Inicio")
        root.geometry("120x45")
        root.configure(bg= "black")
        img = Image.open("titulo.png")
        self.imga = ImageTk.PhotoImage(img)
        Button(root, text="Jugar", command=root.destroy, bg='yellow').pack()  
        root.mainloop()
        
        #Tabla principal
        ventana = tkinter.Tk()
        ventana.title("Ta-Te-Ti")
        ventana.geometry("205x215")
        ...
        img = img.resize((75,63), Image.Resampling.LANCZOS)
        self.imga = ImageTk.PhotoImage(img)
        self.turno = "X"
        img1 = Image.open("O.png")
        img1 = img1.resize((75,63), Image.Resampling.LANCZOS)
