...
class Tateti:
    def __init__(self):
        ...
        root = Tk()
        root.title("Inicio")
        root.geometry("120x95")
        root.configure(bg= "black")
        Button(root, text="3x3", command=root.destroy, bg='yellow').pack()
        Button(root, text="4x4", command=root.destroy, bg='yellow').pack()
        Button(root, text="5x5", command=root.destroy, bg='yellow').pack()
        root.mainloop()
